document.addEventListener("DOMContentLoaded", () => {
    ...
    const minImages = 4; // Mínimo de imágenes simultáneas
    ...
    function getRandomImage() {
        if (usedImages.length === imageCount) {
            usedImages = []; // Reset cuando se han usado todas las imágenes
        }
    ...
    function showRandomImages() {
        ...
        // Programar eliminación de imágenes después de un tiempo aleatorio
        const duration = Math.random() * (maxImageDuration - minImageDuration) + minImageDuration;
    ...
    function startImageCycle() {
        showRandomImages();
        // Actualizar las imágenes cada cierto intervalo
        setInterval(showRandomImages, Math.max(minImageDuration, maxImageDuration * 0.75));
    }
...
document.getElementById('screenshotButton').addEventListener('click', function() {
    const element = document.getElementById('image-container'); // La capa con z-index: 1000

    html2canvas(element).then(canvas => {
        const link = document.createElement('a');
        link.href = canvas.toDataURL('image/jpeg'); // Convierte a formato JPG
        link.download = 'screenshot.jpg'; // Nombre del archivo de descarga
        link.click();
    }).catch(error => {
